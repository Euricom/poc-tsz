...
- Response DTO projects each `UserLeave.Name` and `Allowed` through its `LeaveType` relationship and computes `BalanceDays` (`Total − Taken`; `null` when Unlimited). Nothing is stored that the DTO derives.
- Validation via the existing `ValidationFilter<T>` pattern.

Frontend:

- New `/admin/users` route under `_authed/` with a list view (DataTable) and an Add button that opens the create form.
- Co-located `-components/` folder for the user form and its Zod schema.
...
- [ ] `POST /api/users` returns the created user with one `UserLeave` per non-archived `LeaveType`, all tagged with the current year, `TotalDays` matching the type's `DefaultTotalDays` (`null` for Unlimited), `TakenDays = 0`.
