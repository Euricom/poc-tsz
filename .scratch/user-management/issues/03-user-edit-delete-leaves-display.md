...
- Endpoints, all `RequireAuthorization()`. Same DTO projection rules as slice 02: `Name`, `Allowed`, `Color`, and `IsArchived` projected from the `LeaveType` relationship; `BalanceDays` computed; `null` for Unlimited. (`IsArchived` was added to `UserLeave` in slice 02 — no DTO change needed here.)
